...
export default function Alerts() {
  ...
  const [hpiValue, setHpiValue] = useState("");
  ...
  const sendHPIAlert = async () => {
    ...
    try {
      ...
      const backendURL = "https://sih25067-prototype-3.onrender.com";

      const response = await fetch(`${backendURL}/send_alert`, {
        method: "POST",
      ...
      setStatus("❌ Failed to connect to backend. Check server status.");
    } finally {
      setIsSending(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-xl p-6 border border-blue-100">
        ...
        <div className="bg-white p-6 rounded-xl border border-blue-200 shadow-md mb-6">
          ...
          <input
            type="number"
            step="0.01"
            placeholder="Enter HPI value (e.g., 0.016 for Critical)"
            value={hpiValue}
          ...
          {status && (
            <p className={`mt-3 text-sm flex items-center ${status.startsWith("✅") ? "text-green-700" : "text-red-700"}`}>
              {status.startsWith("✅") ? <CheckCircle size={16} className="mr-2" /> : <XCircle size={16} className="mr-2" />}
              {status}
            </p>
          )}
        </div>
